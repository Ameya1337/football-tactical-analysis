...
def plot_frame(
    ...
    for team, color in zip([hometeam, awayteam], team_colors):
        x_columns = [
            c for c in team.keys() if c[-2:].lower() == "_x" and c != "ball_x"
        ]  # column header for player x positions
        y_columns = [
            c for c in team.keys() if c[-2:].lower() == "_y" and c != "ball_y"
        ...
        if include_player_velocities:
            vx_columns = [
                "{}_vx".format(c[:-2]) for c in x_columns
            ]  # column header for player x positions
            vy_columns = [
                "{}_vy".format(c[:-2]) for c in y_columns
            ]  # column header for player y positions
            ax.quiver(
                team[x_columns],
                team[y_columns],
        ...
        if annotate:
            [
                ...
                if not (np.isnan(team[x]) or np.isnan(team[y]))
            ]
    # plot ball
    ax.plot(
        hometeam["ball_x"],
...
def save_match_clip(
    ...
    with writer.saving(fig, fname, 100):
        for i in index:
            ...
            objs = ax.text(-2.5, field_dimen[1] / 2.0 + 1.0, timestring, fontsize=14)
            figobjs.append(objs)
            writer.grab_frame()
...
def plot_events(
    ...
    for i, row in events.iterrows():
        ...
        if annotate:
            ...
            ax.text(
                row["Start X"], row["Start Y"], textstring, fontsize=10, color=color
            )
    return fig, ax
